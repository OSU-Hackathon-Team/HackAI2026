...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body::after {
    content:""; position:fixed; inset:0;
    background: radial-gradient(ellipse 65% 50% at 15% 20%, rgba(34,211,238,0.07) 0%, transparent 60%),
                radial-gradient(ellipse 60% 50% at 85% 75%, rgba(129,140,248,0.08) 0%, transparent 60%);
    pointer-events:none; z-index:0; animation:nebulaDrift 18s ease-in-out infinite alternate;
  }

  @keyframes nebulaDrift { 0%{opacity:.8;transform:scale(1);} 100%{opacity:1;transform:scale(1.04);} }
  @keyframes blink { 0%,100%{opacity:1;} 50%{opacity:.25;} }
  @keyframes fadeUp { from{opacity:0;transform:translateY(28px);} to{opacity:1;transform:translateY(0);} }
  @keyframes floatA { 0%,100%{transform:scale(1) translate(-50%,-50%);opacity:.5;} 50%{transform:scale(1.12) translate(-46%,-46%);opacity:.85;} }
  @keyframes floatB { 0%,100%{transform:scale(1) translate(-50%,-50%);opacity:.35;} 50%{transform:scale(1.18) translate(-54%,-54%);opacity:.65;} }
  @keyframes scanLine { 0%{transform:translateY(-100%);opacity:0;} 8%{opacity:.5;} 92%{opacity:.5;} 100%{transform:translateY(100vh);opacity:0;} }
  @keyframes gridFadeIn { from{opacity:0;} to{opacity:1;} }
  @keyframes mockSlideUp { from{opacity:0;transform:translateY(60px);} to{opacity:1;transform:translateY(0);} }
  @keyframes marquee { 0%{transform:translateX(0);} 100%{transform:translateX(-50%);} }

  .hero-grid { opacity:0; animation:gridFadeIn 2.5s ease .3s forwards; }
  .scan-ln   { animation:scanLine 4s ease-in-out .8s forwards; }
  .h-badge   { opacity:0; animation:fadeUp 1s cubic-bezier(.16,1,.3,1) .5s forwards; }
  .h-h1      { opacity:0; animation:fadeUp 1.1s cubic-bezier(.16,1,.3,1) .85s forwards; }
  .h-sub     { opacity:0; animation:fadeUp 1s cubic-bezier(.16,1,.3,1) 1.3s forwards; }
  .h-cta     { opacity:0; animation:fadeUp 1s cubic-bezier(.16,1,.3,1) 1.65s forwards; }
  .h-mock    { opacity:0; animation:mockSlideUp 1.3s cubic-bezier(.16,1,.3,1) 2.1s forwards; }

  .reveal { opacity:0; transform:translateY(24px); transition:opacity .65s ease-out,transform .65s ease-out; }
  .reveal.in { opacity:1; transform:translateY(0); }

  .grad-text {
    background:linear-gradient(120deg,var(--cyan) 0%,var(--violet) 55%,var(--pink) 100%);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;
  }

  .nav-link { color:var(--dim); text-decoration:none; font-size:.8rem; font-weight:400; font-family:var(--mono); letter-spacing:.06em; transition:color .2s ease; }
  .nav-link:hover { color:var(--cyan); }

  .btn-primary {
    display:inline-flex; align-items:center; gap:.5rem;
    background:linear-gradient(135deg,var(--vdeep),var(--violet));
    color:#fff; font-family:var(--disp); font-weight:800; font-size:.9rem;
    padding:.85rem 1.85rem; border-radius:12px; border:none; cursor:pointer;
    text-decoration:none; letter-spacing:-.01em; transition:all .25s ease;
    box-shadow:0 4px 24px rgba(99,102,241,.35); position:relative; overflow:hidden;
  }
  ...
  .btn-ghost {
    display:inline-flex; align-items:center; gap:.5rem;
    background:transparent; color:var(--muted); font-family:var(--mono); font-weight:400; font-size:.78rem;
    padding:.75rem 1.35rem; border-radius:10px; border:1px solid rgba(99,102,241,.18);
    cursor:pointer; text-decoration:none; transition:all .2s ease;
  }
  .btn-ghost:hover { border-color:rgba(34,211,238,.35); color:var(--cyan); background:rgba(34,211,238,.05); }
  ...
  .r-card::before { content:''; position:absolute; inset:0; background:linear-gradient(145deg,rgba(99,102,241,.04) 0%,transparent 55%); border-radius:20px; pointer-events:none; }

  .sec-label {
    font-size:.58rem; font-family:var(--mono); letter-spacing:.2em; text-transform:uppercase;
    color:var(--cyan); margin-bottom:.9rem; display:flex; align-items:center; gap:.6rem;
  }
  .sec-label::before { content:""; display:block; width:18px; height:1px; background:linear-gradient(90deg,var(--cyan),transparent); }

  .faq-row { border-bottom:1px solid rgba(99,102,241,.1); padding:1.5rem 0; }
  .faq-btn { width:100%; display:flex; justify-content:space-between; align-items:center; background:none; border:none; cursor:pointer; text-align:left; gap:1.5rem; }

  .mq-wrap { display:flex; width:max-content; animation:marquee 46s linear infinite; }
  .mq-wrap:hover { animation-play-state:paused; }
  .t-card { transition:transform .3s cubic-bezier(.2,0,.2,1),box-shadow .3s ease; cursor:default; width:340px; flex-shrink:0; margin-right:1.5rem; }
  .t-card:hover { transform:translateY(-10px) scale(1.02) !important; z-index:10; }
...
function Chip({ children, color = "var(--violet)" }: { children: React.ReactNode; color?: string }) {
  return (
    <span style={{ fontFamily: "var(--mono)", fontSize: ".57rem", letterSpacing: ".13em", textTransform: "uppercase", padding: ".22rem .65rem", borderRadius: 7, border: `1px solid ${color}40`, background: `${color}12`, color, fontWeight: 500, display: "inline-block" }}>{children}</span>
  );
}
  ...
  return (
    <div style={{ marginBottom: "1.4rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: ".4rem" }}>
        <span style={{ fontSize: ".72rem", fontFamily: "var(--mono)", color: "var(--dim)", letterSpacing: ".08em", textTransform: "uppercase" }}>{label}</span>
        <span style={{ fontSize: ".72rem", fontFamily: "var(--mono)", color, fontWeight: 600 }}>{value}%</span>
      </div>
      <div style={{ height: "3px", background: "rgba(255,255,255,.05)", borderRadius: 4, overflow: "hidden" }}>
        <div style={{ height: "100%", borderRadius: 4, background: `linear-gradient(90deg,${color}aa,${color})`, width: visible ? `${value}%` : "0%", transition: "width 1.4s cubic-bezier(0.16,1,0.3,1)", boxShadow: `0 0 14px ${color}88` }} />
      </div>
    </div>
  );
}
...
function FeatureCard({ icon, title, desc, accent }: { icon: string; title: string; desc: string; accent: string }) {
  return (
    <div className="r-card" style={{ padding: "2rem" }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = `${accent}40`; el.style.boxShadow = `0 20px 60px ${accent}18`; el.style.transform = "translateY(-6px)"; }}
      onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "rgba(99,102,241,.14)"; el.style.boxShadow = "none"; el.style.transform = "translateY(0)"; }}
    >
      ...
      <div style={{ width: "46px", height: "46px", borderRadius: "13px", marginBottom: "1.35rem", background: `linear-gradient(135deg,${accent}18,${accent}06)`, border: `1px solid ${accent}28`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <img src={icon} alt={title} style={{ width: "22px", height: "22px", objectFit: "contain", filter: "brightness(0) invert(1) opacity(0.85)" }} />
      </div>
      <div style={{ fontFamily: "var(--disp)", fontWeight: 800, fontSize: "1.05rem", marginBottom: ".65rem", color: "var(--white)", letterSpacing: "-.025em" }}>{title}</div>
      <div style={{ fontSize: ".82rem", lineHeight: 1.75, color: "var(--muted)", fontFamily: "var(--mono)" }}>{desc}</div>
    </div>
  );
}
...
function TeamCard({ name, title, domain, accent, imageSrc, skills }: { name: string; title: string; domain: string; accent: string; imageSrc: string; skills: string[] }) {
  return (
    <div className="r-card" style={{ padding: "1.85rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = `${accent}35`; el.style.boxShadow = `0 24px 60px ${accent}15`; el.style.transform = "translateY(-5px)"; }}
      onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "rgba(99,102,241,.14)"; el.style.boxShadow = "none"; el.style.transform = "translateY(0)"; }}
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
        <div style={{ width: "54px", height: "54px", borderRadius: "50%", flexShrink: 0, overflow: "hidden", border: `2px solid ${accent}40`, boxShadow: `0 0 24px ${accent}22` }}>
          <img src={imageSrc} alt={name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
        <div>
          <div style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "1rem", color: "var(--white)", letterSpacing: "-.03em" }}>{name}</div>
          <div style={{ fontFamily: "var(--mono)", fontSize: ".58rem", color: accent, letterSpacing: ".12em", textTransform: "uppercase", marginTop: 2 }}>{title}</div>
        </div>
      </div>
      <p style={{ fontFamily: "var(--mono)", fontSize: ".77rem", color: "var(--dim)", lineHeight: 1.8 }}>{domain}</p>
      <div style={{ display: "flex", flexWrap: "wrap", gap: ".4rem" }}>
        {skills.map(s => (<span key={s} style={{ fontFamily: "var(--mono)", fontSize: ".58rem", letterSpacing: ".06em", padding: ".18rem .6rem", borderRadius: 6, background: `${accent}0c`, border: `1px solid ${accent}22`, color: `${accent}cc` }}>{s}</span>))}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="faq-row">
      <button className="faq-btn" onClick={() => setOpen(!open)}>
        <span style={{ fontFamily: "var(--disp)", fontWeight: 700, fontSize: ".98rem", color: "var(--off)", letterSpacing: "-.02em" }}>{q}</span>
        <div style={{ width: "28px", height: "28px", borderRadius: "50%", flexShrink: 0, border: "1px solid rgba(34,211,238,.28)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--cyan)", fontSize: "1rem", transition: "transform .25s, background .25s", transform: open ? "rotate(45deg)" : "rotate(0deg)", background: open ? "rgba(34,211,238,.08)" : "transparent" }}>+</div>
      </button>
      <div style={{ overflow: "hidden", maxHeight: open ? "200px" : "0", transition: "max-height .4s cubic-bezier(0.16,1,0.3,1)" }}>
        <p style={{ paddingTop: "1rem", fontFamily: "var(--mono)", fontSize: ".83rem", lineHeight: 1.85, color: "var(--dim)" }}>{a}</p>
      </div>
    </div>
  );
}
...
function MockUI() {
  return (
    <div style={{ width: "100%", maxWidth: "900px", margin: "0 auto" }}>
      <div style={{ background: "rgba(8,12,28,.97)", borderRadius: "22px", border: "1px solid rgba(99,102,241,.18)", boxShadow: "0 32px 100px rgba(99,102,241,.15), 0 0 0 1px rgba(255,255,255,.03)", overflow: "hidden", backdropFilter: "blur(20px)" }}>
        <div style={{ background: "rgba(255,255,255,.025)", borderBottom: "1px solid rgba(99,102,241,.1)", padding: ".9rem 1.5rem", display: "flex", alignItems: "center", gap: ".6rem" }}>
          {["#ff5f57", "#febc2e", "#28c840"].map(c => <div key={c} style={{ width: "10px", height: "10px", borderRadius: "50%", background: c }} />)}
          <div style={{ flex: 1, textAlign: "center", fontFamily: "var(--mono)", fontSize: ".68rem", color: "var(--dim)", letterSpacing: ".08em" }}>aceit.app/interview</div>
          <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "var(--cyan)", boxShadow: "0 0 8px var(--cyan)", animation: "blink 2s ease-in-out infinite" }} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", minHeight: "320px" }}>
          <div style={{ background: "linear-gradient(135deg,rgba(8,12,28,1) 0%,rgba(15,22,50,.9) 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2.5rem", gap: "1.25rem", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", width: "320px", height: "320px", borderRadius: "50%", background: "radial-gradient(circle,rgba(34,211,238,.05) 0%,transparent 70%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none" }} />
            <div style={{ width: "90px", height: "90px", borderRadius: "50%", background: "linear-gradient(135deg,var(--vdeep),var(--cyan))", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 50px rgba(34,211,238,.25),0 0 100px rgba(99,102,241,.15)" }}>
              <img src="/robot.png" alt="AI" style={{ width: "52px", height: "52px", objectFit: "contain", filter: "brightness(0) invert(1) opacity(.9)" }} />
            </div>
            <div style={{ fontFamily: "var(--disp)", fontWeight: 800, fontSize: ".92rem", color: "var(--off)", letterSpacing: "-.02em" }}>AI Interviewer</div>
            <div style={{ background: "rgba(34,211,238,.07)", border: "1px solid rgba(34,211,238,.2)", borderRadius: "99px", padding: ".3rem 1rem", fontFamily: "var(--mono)", fontSize: ".6rem", color: "var(--cyan)", letterSpacing: ".1em", display: "flex", alignItems: "center", gap: ".4rem" }}>
              <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "var(--cyan)", animation: "blink 1.5s ease-in-out infinite" }} />SPEAKING
            </div>
            <div style={{ display: "flex", gap: "1.75rem", marginTop: ".5rem" }}>
              {[["GAZE", "88", "var(--cyan)"], ["CONF", "82", "var(--violet)"], ["CALM", "91", "var(--green)"]].map(([l, v, c]) => (
                <div key={l} style={{ textAlign: "center" }}>
                  <svg width="54" height="54" viewBox="0 0 54 54">
                    <circle cx="27" cy="27" r="21" fill="none" stroke="rgba(255,255,255,.05)" strokeWidth="3" />
                    <circle cx="27" cy="27" r="21" fill="none" stroke={c} strokeWidth="3" strokeDasharray={`${2 * Math.PI * 21 * parseInt(v) / 100} ${2 * Math.PI * 21}`} strokeLinecap="round" transform="rotate(-90 27 27)" style={{ filter: `drop-shadow(0 0 4px ${c})` }} />
                    <text x="27" y="32" textAnchor="middle" fill={c} fontSize="10" fontFamily="monospace" fontWeight="700">{v}</text>
                  </svg>
                  <div style={{ fontFamily: "monospace", fontSize: ".5rem", color: "var(--dim)", letterSpacing: ".14em", marginTop: 2 }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: ".8rem", borderLeft: "1px solid rgba(99,102,241,.1)" }}>
            <div style={{ fontFamily: "var(--mono)", fontSize: ".56rem", letterSpacing: ".16em", color: "var(--dim)", textTransform: "uppercase", marginBottom: ".25rem" }}>Live Transcript</div>
            {[
              { s: "AI", t: "Tell me about a challenging project you've worked on.", c: "var(--cyan)" },
              { s: "YOU", t: "Sure — I built a real-time pipeline processing 10k events/sec...", c: "var(--violet)" },
              { s: "AI", t: "How did you handle failure states?", c: "var(--cyan)" },
            ].map((m, i) => (
              <div key={i} style={{ display: "flex", gap: ".6rem", padding: ".65rem .8rem", borderRadius: 12, background: "rgba(255,255,255,.02)", border: "1px solid rgba(99,102,241,.07)" }}>
                <span style={{ fontFamily: "var(--mono)", fontSize: ".56rem", fontWeight: 700, color: m.c, paddingTop: 3, flexShrink: 0, letterSpacing: ".08em" }}>{m.s}</span>
                <p style={{ fontFamily: "var(--mono)", fontSize: ".72rem", lineHeight: 1.6, color: "var(--dim)" }}>{m.t}</p>
              </div>
            ))}
            <div style={{ marginTop: "auto", background: "rgba(251,113,133,.07)", border: "1px solid rgba(251,113,133,.2)", borderRadius: 10, padding: ".6rem .875rem", fontFamily: "var(--mono)", fontSize: ".63rem", color: "var(--danger)" }}>⚠ Maintain eye contact</div>
          </div>
        </div>
      </div>
    </div>
...
const TESTIMONIALS = [
  { initial: "S", name: "Suchith M.", role: "ECE Student → NVIDIA Intern", accent: "var(--cyan)", text: "The gaze feedback was eye-opening. I didn't realize I look away when I'm thinking — now I consciously anchor my eyes and it makes a huge difference." },
  { initial: "J", name: "Jacob B.", role: "Career Switcher → SWE at Stripe", accent: "var(--violet)", text: "Being able to click on a stress spike and immediately hear what I was saying at that moment is genuinely brilliant." },
  { initial: "C", name: "Cooper B.", role: "CS Student", accent: "var(--green)", text: "I went in cold for my first interview and bombed it. After three sessions with AceIt my confidence scores jumped from 54 to 83." },
  { initial: "D", name: "Darquavius S.", role: "Masters Student → Meta", accent: "var(--danger)", text: "The AI interviewer asked me things I didn't expect — it had clearly read my resume carefully. Much harder than the real interview, honestly." },
  { initial: "A", name: "Aisha K.", role: "PM → Google L4", accent: "var(--warn)", text: "I used to freeze when asked about weaknesses. AceIt replayed three moments where my voice dropped — and helped me reframe them completely." },
  { initial: "R", name: "Rohan P.", role: "Bootcamp Grad → Shopify", accent: "var(--cyan)", text: "Seeing my fidget index spike exactly when I was asked a system design question was humbling. But also incredibly useful." },
];

function TestimonialsSection() {
  return (
    <section style={{ position: "relative", padding: "9rem 0", overflow: "hidden", borderTop: "1px solid rgba(99,102,241,.08)", borderBottom: "1px solid rgba(99,102,241,.08)", minHeight: "580px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "radial-gradient(ellipse 60% 60% at 20% 50%,rgba(34,211,238,.03) 0%,transparent 60%),radial-gradient(ellipse 60% 60% at 80% 50%,rgba(129,140,248,.04) 0%,transparent 60%)" }} />
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", marginBottom: "4.5rem", padding: "0 2rem" }}>
        <div className="sec-label" style={{ justifyContent: "center" }}>04 — Reviews</div>
        <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.5vw,3rem)", letterSpacing: "-.04em", color: "var(--white)" }}>Success from the source</h2>
        <p style={{ fontFamily: "var(--mono)", fontSize: ".78rem", color: "var(--dim)", marginTop: ".75rem" }}>Hover any card to pause</p>
      </div>
      <div style={{ position: "relative", width: "100%", overflow: "hidden" }}>
        <div className="mq-wrap">
          {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
            <div key={i} className="t-card r-card" style={{ padding: "1.75rem" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = `0 24px 60px ${t.accent}28`; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = "none"; }}
            >
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "1px", background: `linear-gradient(90deg,transparent,${t.accent}55,transparent)` }} />
              <div style={{ display: "flex", alignItems: "center", gap: ".875rem", marginBottom: "1.25rem" }}>
                <div style={{ width: "42px", height: "42px", borderRadius: "50%", background: `${t.accent}12`, border: `1px solid ${t.accent}30`, display: "flex", alignItems: "center", justifyContent: "center", color: t.accent, fontFamily: "var(--disp)", fontWeight: 900, fontSize: "1rem", flexShrink: 0 }}>{t.initial}</div>
                <div>
                  <div style={{ fontFamily: "var(--disp)", fontWeight: 700, fontSize: ".88rem", color: "var(--off)", letterSpacing: "-.02em" }}>{t.name}</div>
                  <div style={{ fontFamily: "var(--mono)", fontSize: ".6rem", color: "var(--dim)", marginTop: 3 }}>{t.role}</div>
                </div>
              </div>
              <p style={{ fontFamily: "var(--mono)", fontSize: ".78rem", lineHeight: 1.8, color: "var(--muted)" }}>"{t.text}"</p>
            </div>
          ))}
        </div>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 20, background: "linear-gradient(to right,var(--bg) 0%,transparent 12%,transparent 88%,var(--bg) 100%)" }} />
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const onScroll = () => {
      if (!heroMockRef.current) return;
      heroMockRef.current.style.transform = `translateY(-${Math.min(window.scrollY * 0.27, 80)}px)`;
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => window.removeEventListener("scroll", onScroll);
  }, []);
  ...
  return (
    <>
      ...
      {/* ── NAV ── */}
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,11,24,.82)", backdropFilter: "blur(20px) saturate(1.4)", borderBottom: "1px solid rgba(99,102,241,.1)", padding: "0 3rem", height: "68px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "1.35rem", letterSpacing: "-.04em" }}>
          Ace<span className="grad-text">It</span>
        </div>
        <div style={{ display: "flex", gap: "2.5rem" }}>
          {[["#features", "Features"], ["#feedback", "How it works"], ["#team", "Team"], ["#faq", "FAQ"]].map(([href, label]) => (
      ...
      {/* ── HERO ── */}
      <section style={{ minHeight: "100vh", position: "relative", display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "center", padding: "8rem 3rem 4rem", overflow: "hidden" }}>
        <div className="hero-grid" style={{ position: "absolute", inset: 0, zIndex: 0, backgroundImage: "linear-gradient(rgba(99,102,241,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(99,102,241,.04) 1px,transparent 1px)", backgroundSize: "72px 72px", maskImage: "radial-gradient(ellipse 90% 80% at 35% 40%,black 10%,transparent 80%)", WebkitMaskImage: "radial-gradient(ellipse 90% 80% at 35% 40%,black 10%,transparent 80%)" }} />
        <div className="scan-ln" style={{ position: "absolute", left: 0, right: 0, height: "1px", zIndex: 1, background: "linear-gradient(90deg,transparent,rgba(34,211,238,.6),rgba(129,140,248,.5),transparent)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", width: "700px", height: "700px", borderRadius: "50%", zIndex: 0, background: "radial-gradient(circle,rgba(34,211,238,.08) 0%,transparent 65%)", top: "10%", left: "50%", transformOrigin: "0 0", animation: "floatA 7s ease-in-out infinite", pointerEvents: "none" }} />
        <div style={{ position: "absolute", width: "550px", height: "550px", borderRadius: "50%", zIndex: 0, background: "radial-gradient(circle,rgba(129,140,248,.1) 0%,transparent 65%)", top: "28%", left: "63%", transformOrigin: "0 0", animation: "floatB 10s ease-in-out infinite", pointerEvents: "none" }} />

        <div style={{ position: "relative", zIndex: 2, maxWidth: "780px" }}>
          <div className="h-badge" style={{ display: "inline-flex", alignItems: "center", gap: ".6rem", background: "rgba(34,211,238,.06)", border: "1px solid rgba(34,211,238,.18)", borderRadius: "8px", padding: ".35rem 1rem", marginBottom: "2.5rem" }}>
            <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "var(--cyan)", display: "inline-block", boxShadow: "0 0 8px var(--cyan)", animation: "blink 2s ease-in-out infinite" }} />
            <span style={{ fontFamily: "var(--mono)", fontSize: ".66rem", color: "var(--cyan)", letterSpacing: ".1em" }}>MULTIMODAL CONFIDENCE ANALYSIS — NOW LIVE</span>
          </div>

          <h1 className="h-h1" style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(3.5rem,7.5vw,6.5rem)", lineHeight: .97, letterSpacing: "-.05em", color: "var(--white)", marginBottom: "1.75rem" }}>
            Ace your<br /><span className="grad-text">next interview.</span>
          </h1>

          <p className="h-sub" style={{ fontFamily: "var(--mono)", fontSize: "1rem", color: "var(--dim)", maxWidth: "460px", lineHeight: 1.85, marginBottom: "3rem" }}>
            AI-powered mock interviews that analyze your voice, gaze, and body language in real-time — then coach you to close the gap.
          </p>

          <div className="h-cta" style={{ display: "flex", gap: "1rem", alignItems: "center", marginBottom: "6rem" }}>
            <Link href="/interviewer-selection" className="btn-primary">Start my interview →</Link>
            <span style={{ fontFamily: "var(--mono)", fontSize: ".72rem", color: "rgba(99,102,241,.4)" }}>No account needed</span>
          </div>
        </div>

        <div className="h-mock" ref={heroMockRef} style={{ width: "100%", position: "relative", zIndex: 2 }}>
          <MockUI />
        </div>
      </section>

      {/* ── STATS STRIP ── */}
      <div style={{ borderTop: "1px solid rgba(99,102,241,.1)", borderBottom: "1px solid rgba(99,102,241,.1)", padding: "2.5rem 3rem", display: "flex", justifyContent: "center", gap: "5rem", background: "rgba(99,102,241,.02)", position: "relative", zIndex: 1 }}>
        {[["10k+", "Interviews completed"], ["94%", "Report accuracy"], ["<500ms", "AI response time"], ["3×", "Faster offer rates"]].map(([val, label]) => (
          <div key={label} style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "1.9rem", letterSpacing: "-.04em" }} className="grad-text">{val}</div>
            <div style={{ fontFamily: "var(--mono)", fontSize: ".65rem", color: "var(--dim)", marginTop: ".3rem", letterSpacing: ".06em" }}>{label}</div>
          </div>
        ))}
      </div>

      {/* ── PERSONALIZED ── */}
      <section id="features" style={{ padding: "11rem 3rem", maxWidth: "1240px", margin: "0 auto" }}>
        <div ref={personalizedRef.ref} className={`reveal ${personalizedRef.visible ? "in" : ""}`} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6rem", alignItems: "center" }}>
          <div className="r-card" style={{ padding: "2rem" }}>
            ...
            <div style={{ fontFamily: "var(--mono)", fontSize: ".57rem", color: "var(--dim)", marginBottom: "1.25rem", letterSpacing: ".16em", textTransform: "uppercase" }}>Session Context</div>
            <div style={{ display: "flex", flexDirection: "column", gap: ".875rem" }}>
              {[
                { icon: "/file.png", title: "CV uploaded", sub: "resume_2025.pdf · 142 KB", color: "var(--cyan)" },
                { icon: "/briefcase.png", title: "Job listing imported", sub: "Software Engineer · Stripe", color: "var(--green)" },
              ].map(item => (
                <div key={item.title} style={{ display: "flex", alignItems: "center", gap: ".875rem", background: "rgba(99,102,241,.04)", borderRadius: 13, padding: ".875rem 1rem", border: "1px solid rgba(99,102,241,.1)" }}>
                  <div style={{ width: "36px", height: "36px", borderRadius: 10, background: `${item.color}10`, border: `1px solid ${item.color}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <img src={item.icon} alt={item.title} style={{ width: "18px", height: "18px", objectFit: "contain", filter: "brightness(0) invert(1) opacity(.8)" }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: "var(--disp)", fontWeight: 700, fontSize: ".84rem", color: "var(--off)", letterSpacing: "-.02em" }}>{item.title}</div>
                    <div style={{ fontFamily: "var(--mono)", fontSize: ".64rem", color: "var(--dim)" }}>{item.sub}</div>
                  </div>
                  <div style={{ marginLeft: "auto", fontSize: ".82rem", color: "var(--green)" }}>✓</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: "1.25rem", padding: ".875rem 1rem", background: "rgba(34,211,238,.04)", borderRadius: 13, border: "1px dashed rgba(34,211,238,.18)" }}>
              <div style={{ fontFamily: "var(--mono)", fontSize: ".68rem", color: "var(--cyan)" }}>✦ Generating bespoke interview persona...</div>
            </div>
          </div>
          <div>
            <div className="sec-label">01 — Personalization</div>
            <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.2vw,2.9rem)", lineHeight: 1.08, letterSpacing: "-.04em", color: "var(--white)", marginBottom: "1.25rem" }}>Instantly personalised interviews</h2>
            <p style={{ fontFamily: "var(--mono)", fontSize: ".87rem", color: "var(--dim)", lineHeight: 1.9, marginBottom: "2rem" }}>Upload your resume and paste any job description. AceIt generates a bespoke interviewer with tailored technical and behavioral questions — specific to the exact role.</p>
            <Link href="/interviewer-selection" className="btn-primary" style={{ fontSize: ".85rem", padding: ".72rem 1.4rem" }}>Try it now →</Link>
          </div>
        </div>
      </section>

      {/* ── FEEDBACK ── */}
      <section id="feedback" style={{ padding: "9rem 3rem", borderTop: "1px solid rgba(99,102,241,.08)", borderBottom: "1px solid rgba(99,102,241,.08)", background: "rgba(99,102,241,.02)" }}>
        <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
          <div ref={feedbackRef.ref} className={`reveal ${feedbackRef.visible ? "in" : ""}`} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6rem", alignItems: "center" }}>
            <div>
              <div className="sec-label">02 — Feedback</div>
              <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.2vw,2.9rem)", lineHeight: 1.08, letterSpacing: "-.04em", color: "var(--white)", marginBottom: "1.25rem" }}>Actionable, timestamped coaching</h2>
              <p style={{ fontFamily: "var(--mono)", fontSize: ".87rem", color: "var(--dim)", lineHeight: 1.9 }}>Every session produces a timestamped report that cross-references your transcript with biometric signals — so you know exactly when and why your confidence dipped.</p>
            </div>
            <div className="r-card" style={{ padding: "2.5rem" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "1px", background: "linear-gradient(90deg,transparent,var(--violet),transparent)" }} />
              <div style={{ fontFamily: "var(--mono)", fontSize: ".56rem", color: "var(--dim)", marginBottom: "2rem", letterSpacing: ".16em", textTransform: "uppercase" }}>Performance Breakdown</div>
              <SkillBar label="Communication" value={87} color="var(--cyan)" visible={feedbackRef.visible} />
              <SkillBar label="Confidence" value={74} color="var(--violet)" visible={feedbackRef.visible} />
              <SkillBar label="Problem solving" value={91} color="var(--green)" visible={feedbackRef.visible} />
              <SkillBar label="Technical ability" value={82} color="var(--pink)" visible={feedbackRef.visible} />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── FEATURES GRID ── */}
      <section style={{ padding: "11rem 3rem", maxWidth: "1240px", margin: "0 auto" }}>
        <div ref={featuresRef.ref} className={`reveal ${featuresRef.visible ? "in" : ""}`}>
          <div style={{ textAlign: "center", marginBottom: "5rem" }}>
            <div className="sec-label" style={{ justifyContent: "center" }}>03 — Features</div>
            <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.5vw,3rem)", lineHeight: 1.08, letterSpacing: "-.04em", color: "var(--white)" }}>Practice makes perfect</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: "1.25rem" }}>
            <FeatureCard icon="/microphone.png" title="Voice chat" accent="var(--cyan)" desc="Speak naturally with an AI interviewer powered by GPT-4o and ElevenLabs — sub-500ms response time, no typing required." />
            <FeatureCard icon="/robot.png" title="Powerful interviewer" accent="var(--violet)" desc="Your resume and job description shape a bespoke interviewer that asks questions specific to your experience and the role." />
            <FeatureCard icon="/bar-chart.png" title="Constructive feedback" accent="var(--green)" desc="Timestamped coaching tied to the exact moment you looked away, rushed your words, or showed stress in your posture." />
            <FeatureCard icon="/chart.png" title="Get better" accent="var(--pink)" desc="Track gaze stability, voice confidence, and composure across sessions and watch your scores improve over time." />
          </div>
        </div>
      ...
      {/* ── TEAM ── */}
      <section id="team" style={{ padding: "11rem 3rem", maxWidth: "1240px", margin: "0 auto" }}>
        <div ref={teamRef.ref} className={`reveal ${teamRef.visible ? "in" : ""}`}>
          <div style={{ textAlign: "center", marginBottom: "5rem" }}>
            <div className="sec-label" style={{ justifyContent: "center" }}>05 — Team</div>
            <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.5vw,3rem)", letterSpacing: "-.04em", color: "var(--white)", marginBottom: "1rem" }}>Built by engineers,<br />for candidates.</h2>
            <p style={{ fontFamily: "var(--mono)", fontSize: ".8rem", color: "var(--dim)", maxWidth: "420px", margin: "0 auto", lineHeight: 1.85 }}>Four people. One hackathon. A shared belief that interview prep should be as sophisticated as the interviews themselves.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: "1.25rem" }}>
            <TeamCard name="Konrád" imageSrc="/konrad.png" title="Systems & Frontend Architect" accent="var(--cyan)" domain="Builds the infrastructure that holds everything together — WebRTC signaling, global state orchestration, and the synchronized analytics dashboard." skills={["Next.js", "WebRTC", "Zustand", "Recharts", "Supabase"]} />
            <TeamCard name="Jason" imageSrc="/jason.png" title="Audio & Intelligence Orchestrator" accent="var(--violet)" domain="Runs the conversational brain — RAG ingestion, cascading GPT-4o streams, ElevenLabs voice synthesis, and Whisper word-level timestamps." skills={["FastAPI", "GPT-4o", "Whisper", "ElevenLabs", "LangChain"]} />
            <TeamCard name="Mitch" imageSrc="/mitch.png" title="Computer Vision Engineer" accent="var(--green)" domain="Extracts meaning from 468 facial landmarks in real time — gaze vectors, head pose, and fidget index — streamed live via WebRTC DataChannels." skills={["MediaPipe", "OpenCV", "Flask", "PyTorch", "NumPy"]} />
            <TeamCard name="Rocky" imageSrc="/rocky.png" title="Business Lead" accent="var(--pink)" domain="Keeps the team pointed at what matters — market positioning, pitch deck, user testimonials, and making sure the demo lands with the judges." skills={["Strategy", "Pitch Deck", "User Research", "Data"]} />
          </div>
        </div>
      ...
      {/* ── FAQ ── */}
      <section id="faq" style={{ padding: "9rem 3rem", borderTop: "1px solid rgba(99,102,241,.08)", borderBottom: "1px solid rgba(99,102,241,.08)", background: "rgba(99,102,241,.02)" }}>
        <div style={{ maxWidth: "700px", margin: "0 auto" }}>
          <div ref={faqRef.ref} className={`reveal ${faqRef.visible ? "in" : ""}`}>
            <div style={{ textAlign: "center", marginBottom: "4rem" }}>
              <div className="sec-label" style={{ justifyContent: "center" }}>06 — FAQ</div>
              <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(2rem,3.2vw,2.9rem)", letterSpacing: "-.04em", color: "var(--white)" }}>Common questions</h2>
            </div>
            <FaqItem q="How does the AI analyze my body language?" a="AceIt runs MediaPipe facial landmark detection in your browser at 30fps, tracking 468 3D points to derive gaze direction, head pose, and fidget index — all processed locally with no video leaving your device." />
            <FaqItem q="What kind of questions will I be asked?" a="Questions are generated by GPT-4o based on your specific resume and job posting. Expect a mix of behavioral (STAR-format) and technical questions tailored to the role and your experience level." />
            <FaqItem q="Is my video or audio stored?" a="No. All biometric processing happens in your browser. Only anonymized metric scores (gaze, confidence, fidget index) are stored in your session — never raw video or audio." />
      ...
      {/* ── FINAL CTA ── */}
      <section style={{ background: "var(--bg)", padding: "13rem 3rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: "800px", height: "800px", borderRadius: "50%", background: "radial-gradient(circle,rgba(34,211,238,.05) 0%,transparent 65%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", width: "550px", height: "550px", borderRadius: "50%", background: "radial-gradient(circle,rgba(129,140,248,.07) 0%,transparent 65%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none" }} />
        <div ref={ctaRef.ref} className={`reveal ${ctaRef.visible ? "in" : ""}`} style={{ position: "relative", zIndex: 1 }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: ".58rem", color: "var(--cyan)", letterSpacing: ".2em", textTransform: "uppercase", marginBottom: "2rem", display: "flex", alignItems: "center", justifyContent: "center", gap: ".6rem" }}>
            <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "var(--cyan)", display: "inline-block", animation: "blink 2s ease-in-out infinite" }} />
            Ready when you are
          </div>
          <h2 style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "clamp(3rem,7vw,6.5rem)", letterSpacing: "-.05em", color: "var(--white)", marginBottom: "1.25rem", lineHeight: .98 }}>
            Get the job of<br /><span className="grad-text">your dreams.</span>
          </h2>
          <p style={{ fontFamily: "var(--mono)", fontSize: ".88rem", color: "var(--dim)", marginBottom: "3rem" }}>No fluff. No filler. Just better interviews.</p>
          <Link href="/interviewer-selection" className="btn-primary" style={{ fontSize: "1rem", padding: "1.1rem 2.75rem" }}>Start for free →</Link>
        </div>
      </section>

      {/* ── FOOTER ── */}
      <footer style={{ background: "rgba(99,102,241,.02)", borderTop: "1px solid rgba(99,102,241,.1)", padding: "2.5rem 3rem" }}>
        <div style={{ maxWidth: "1240px", margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          <div style={{ fontFamily: "var(--disp)", fontWeight: 900, fontSize: "1.25rem", letterSpacing: "-.04em" }}>
            Ace<span className="grad-text">It</span>
          </div>
          <div style={{ display: "flex", gap: "2.5rem" }}>
            {[["#features", "Features"], ["#team", "Team"], ["#faq", "FAQ"]].map(([href, label]) => (
              <a key={href} href={href} className="nav-link">{label}</a>
            ))}
            <Link href="/interviewer-selection" className="nav-link">Start interview</Link>
          </div>
          <div style={{ fontFamily: "var(--mono)", fontSize: ".65rem", color: "rgba(99,102,241,.25)" }}>© 2026 AceIt · Built for good interviews</div>
        </div>
      </footer>
    </>
  );
